...
class Downloader(ThreadPool):
    ...
    def download(self, task, default_ext, timeout=5, max_retry=3, **kwargs):
        ...
        # 3번까지 시도하고 실패하면 다운로드하지 않고 넘어감
        retry = max_retry

        while retry > 0 and not self.signal.get('reach_max_num'):
            try:
                # 이미지 데이터를 받는 코드. 나한테는 필요없음
                # 만약 이 코드가 실행되면 url에 있는 데이터르 로드하므로 시간 낭비
                response = None     # self.session.get(file_url, timeout=timeout)
                response.status_code = 200
            except Exception as e:
                # self.logger.error('Exception caught when downloading file %s, '
                #                   'error: %s, remaining retry times: %d',
                #                   file_url, e, retry - 1)
                self.logger.info('Get Image Url')
                print(file_url)
                retry = 0
            else:
    ...
    def worker_exec(self,
        ...
        while True:
            ...
            try:
                ...
                self.url_list.append(task['file_url'])
            except queue.Empty:
        ...
        # 장고에서는 쓰레드가 여러 개 돌아가므로 이렇게 종료 조건을 명시해줘야 함
        self.signal.set(finish_work=True)
